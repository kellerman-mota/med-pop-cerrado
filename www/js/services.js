...
.factory('Plantas', function() {
  var plantas = [{
    ...
    indicacoes:'Congestão; Hemorragia (cerebral, pulmonar); Hidropsia (cardíaca, renal); Prisão de ventre; Edemas; Inflamações; Dor de cabeça; Febre.`, uso:`Chá da raiz: Coloque 2 colheres de sopa para um litro de água. Deixe cozinhar por cerca de 10 minutos a partir do momento em que se inicia a ebulição, após esse tempo. Retire do fogo e deixe repousando,tampada, por 10 minutos. Coe e está pronto para o uso.',
  ...
  return {
    ...
    filtrarPorSintoma: function(filtro) {
      if(filtro){
        var results = plantas.filter(function(p){
           return p.indicacoes.indexOf(filtro) > -1;
        });
        return results;
      }else{
        return plantas;
